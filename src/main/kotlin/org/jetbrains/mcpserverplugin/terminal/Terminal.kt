...
class RunAndroidTests : AbstractMcpTool<NoArgs>() {
    ...
    override val description: String = """
        ...
        **REQUIRES** connected Android device or emulator (check with list_devices first).
        Tests run on actual Android system, can test UI interactions and Android-specific functionality.
        AI Agent: Only use this tool when you need to test Android-specific features or UI. Check device connection first.
    """
    ...
    override fun handle(project: Project, args: NoArgs): Response {
        ...
        return executeTerminalCommandTool.handle(
            project,
            ExecuteTerminalCommandArgs("./gradlew connectedDebugAndroidTest --stacktrace")
        )
    }
}
...
@Serializable
data class StartEmulatorArgs(val emulatorName: String)

class StartEmulator : AbstractMcpTool<StartEmulatorArgs>() {
    override val name: String = "start_emulator"
    override val description: String = """
        **SLOW** (60-180 seconds): Starts Android emulator with specified name.
        Emulator takes time to boot completely. Use list_available_emulators to see options.
        **USE THIS** when you need Android device for testing but no physical device available.
        AI Agent: Use this tool when instrumented tests fail due to no connected devices. Takes several minutes to start.
    """
    
    override fun handle(project: Project, args: StartEmulatorArgs): Response {
        val executeTerminalCommandTool = ExecuteTerminalCommandTool()
        return executeTerminalCommandTool.handle(
            project,
            ExecuteTerminalCommandArgs("""
                if [ -n "${'$'}ANDROID_HOME" ]; then
                    ${'$'}ANDROID_HOME/emulator/emulator -avd ${args.emulatorName} &
                elif [ -f ~/Library/Android/sdk/emulator/emulator ]; then
                    ~/Library/Android/sdk/emulator/emulator -avd ${args.emulatorName} &
                else
                    echo "Android emulator not found. Please install Android SDK or set ANDROID_HOME"
                fi
            """.trimIndent())
...
class GitDiff : AbstractMcpTool<NoArgs>() {
    ...
    override val description: String = "**FAST**: Shows current uncommitted changes in the project."
